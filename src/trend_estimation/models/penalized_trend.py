...
from trend_estimation.core.smoothness import smoothness_to_lambda, lambda_to_smoothness
...
class PenalizedTrend(BaseTrendEstimator):
    ...
    def fit(self, y, X=None):
        ...
        self.y_ = y
        self.n_obs_ = y.size
        self.solver_ = GuerreroSpectralSolver(y.size, self.order)
        if self.lambda_ is None:
            if self.smoothness is None:
                raise ValueError("Either smoothness or lambda_ must be provided.")
            lambda_value = smoothness_to_lambda(float(self.smoothness), y.size, self.order)
        else:
